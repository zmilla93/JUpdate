...
foreach($arg in $args){
    if($arg.StartsWith($launcherPrefix)) { $launcher = $arg.Replace($launcherPrefix, "") }
    if($arg.StartsWith($installerPrefix)) { $installer = $arg.Replace($installerPrefix, "") }
}
if ($null -eq $launcher) { throw("No launcher argument specified!") }
if ($null -eq $installer) { throw("No installer argument specified!") }
